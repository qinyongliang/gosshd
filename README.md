...
This path uses prebuilt GitHub Release archives. No local Go toolchain or local build is required.

Start the public server on a Linux/macOS public host. The repository `run.sh` detects the OS/CPU, downloads the latest matching `gosshd-server` release archive into `/tmp`, extracts it, and passes all arguments through to the server:
...
```powershell
$script = irm https://raw.githubusercontent.com/qinyongliang/gosshd/main/run.ps1
& ([scriptblock]::Create($script)) --http-listen :80 --ssh-listen :22 --public-host public-host
```

The normal server archive is lightweight. When a private host requests `/download/agent/{goos}/{goarch}`, the running `gosshd-server` downloads the matching agent binary from its own GitHub Release version, caches it under the system temp directory, and serves it back. If direct GitHub download fails or is slower than 100 KB/s, the server retries through `https://gh-proxy.com/`.

`full` release archives keep the previous all-in-one behavior and include `dist/agent` for offline or restricted-network deployments.
...
```powershell
irm http://public-host/run.ps1 | iex
```
