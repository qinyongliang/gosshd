...
这个方式只使用 GitHub Release 中已经编译好的产物，不需要在本地安装 Go，也不需要本地编译。

在 Linux/macOS 公网主机上启动 server。仓库中的 `run.sh` 会自动识别系统/CPU，下载最新版匹配的 `gosshd-server` Release 压缩包到 `/tmp`，解压后把所有参数透传给 server：
...
普通 server 压缩包不会内置全部 agent。私有网络机器访问正在运行的 `gosshd-server` 的 `/download/agent/{goos}/{goarch}` 时，server 会按需从自身对应版本的 GitHub Release 下载匹配平台的 agent，缓存到系统临时目录，并转发给私网机器。如果直连 GitHub 下载失败或速度低于 100 KB/s，会自动改用 `https://gh-proxy.com/` 重试。

`full` Release 压缩包保留原来的全打包方式，会内置 `dist/agent`，适合离线或受限网络部署。
